...
def calculate_similarity(user_vector: np.ndarray, target_vector: np.ndarray) -> float:
    return cosine_similarity(user_vector.reshape(1, -1), target_vector.reshape(1, -1))[0][0]


def weighted_similarity(user: User, target_user: User) -> float:
...
def get_ranked_users(users: List[User], target_user: User) -> List[User]:
